...
#######################################################
# EDIT NO LONGER ONLY INDEPENDENT SNPS FOR ESTIMATION
#######################################################
#Get LD-independent SNPs for estimation of bivariate params
#for(ch in 1:22){
#	system(paste0("plink --noweb --vcf /project/voight_datasets/1kg/phaseIII/chr",toString(ch),".1kg.phase3.v5.vcf --keep /project/voight_datasets/1kg/phaseIII/plinkfiles/SuperPopLists/EUR  --extract AllRs",ts,".txt --out onlyGWASNPS_",ts,"_ch",toString(ch)," --indep-pairwise 1000 5 1.0 --chr ",toString(ch)))
#}
#system(paste0("cat onlyGWASNPS_",ts,"_ch*.prune.in > onlyGWASNPS_",ts,"_allch.prune.in"))

indRS = data.frame(har$SNP)
...
res = har[har$SNP %in% indRS$har.SNP,]
