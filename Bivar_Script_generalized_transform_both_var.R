...
for(ch in 1:22){
	system(paste0("plink --noweb --vcf /project/voight_datasets/1kg/phaseIII/chr",toString(ch),".1kg.phase3.v5.vcf --keep /project/voight_datasets/1kg/phaseIII/plinkfiles/SuperPopLists/EUR  --extract AllRs",ts,".txt --out onlyGWASNPS_",ts,"_ch",toString(ch)," --indep-pairwise 1000 5 0.2 --chr ",toString(ch)))
}
...
###############################################################################
#These edits are designed to transform both har$z_exp and har$z_out columns to make them standard normal. This is required to generate a variance of 1 for several anthropomorphic traits that seem to be overly genomic corrected
###############################################################################

#calculate mean of out data z-scores
harZoutMean = mean(res$z_out)
harZexpMean = mean(res$z_exp)
# calculated variance and SE
harZoutVar = var(res$z_out)
harZoutSE = sqrt(harZoutVar)

harZexpVar = var(res$z_exp)
harZexpSE = sqrt(harZexpVar)

# perform standardizing transformation
harZoutstnd = (res$z_out - harZoutMean) / harZoutSE

harZexpstnd = (res$z_exp - harZexpMean) / harZexpSE

# replace har$z_out column with harZstnd values
res$z_out <- harZoutstnd

res$z_exp <- harZexpstnd
...
# Added this quit so I can just get the estimated means and var covar matrices
quit()
...
system(paste0("plink --noweb --bfile /project/voight_selscan/ksiewert/CardioMetaAnalysis/LDL_CHD_Bivar/LDClump/PlinkFilesOnlyRs/mergedBed  --keep /project/voight_datasets/1kg/phaseIII/plinkfiles/SuperPopLists/EUR --clump-p1 .000001 --clump-r2 0.2  --clump-kb 1000 --clump AllSNPs_",ts,"_bivarresults.txt --out allChrMergedClumped"))
...
d = d[o,]
#### Doing "Multivariate Scan in Bivariate steps so reduce significance to 1e-5
d = d[which(d$P<1e-5),] #contains all clustered SNP with p<1e-7 whether or not novel
...
GWAS = read.table("/project/voight_datasets/GWAS/15_GWASCatalog/Feb_2018_version/GWAShg37.bed",header=FALSE,sep="\t",colClasses = "character",fill=TRUE,quote = "",stringsAsFactors = FALSE)
...
system(paste0("plink --bfile /project/voight_selscan/ksiewert/CardioMetaAnalysis/LDL_CHD_Bivar/LDClump/PlinkFilesOnlyRs/mergedBed --keep /project/voight_datasets/1kg/phaseIII/plinkfiles/SuperPopLists/EUR --show-tags novelSNPrsNums.txt --out NovelSNPTags  --tag-kb 1000 --list-all --tag-r2 0.2"))
